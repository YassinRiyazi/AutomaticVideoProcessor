...
def EdgePointSorter_Numba(address: str) -> npt.NDArray[np.float64]:
    """
    ...
    i_array, j_array = _edgeSort(data['i_list'], data['j_list'], origin_x=data['i_list'].min())
...
if __name__ == "__main__":
    ...
    address = "/media/d25u2/Dont/RudExp/S1/S1-26-30d-01/databases/frame_000386.png"
    df = pd.read_csv("/media/d25u2/Dont/RudExp/S1/S1-26-30d-01/databases/detections.csv")
    match = df[df['image'] == os.path.basename(address)]
    
    if match.empty:
        raise ValueError(f"Image '{os.path.basename(address)}' not found in the CSV.")
    row = match.iloc[0]
    endpoint = int(row['endpoint'])
    beginning = int(row['beginning'])

    X = EdgePointSorter_Numba(address.replace('.png', '.npz'))
    i_array, j_array = X[:, 0]/3-15, X[:, 1]/3+4


    plt.figure(figsize=(10, 10))
    address = address.replace('databases', 'frames_rotated')
    _image = LoadImage(address)
    plt.imshow(_image[::-1, :], cmap='gray')

    mid = len(i_array) #// 2
    colors = np.linspace(0, 1, mid)
    plt.scatter((i_array[:mid])+endpoint, (j_array[:mid]), c=colors, cmap='RdBu', s=6)
